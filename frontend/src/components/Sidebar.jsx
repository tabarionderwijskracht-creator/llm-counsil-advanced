...
export default function Sidebar({
  ...
  const [editingConversationId, setEditingConversationId] = useState(null); // conversation id being renamed
  ...
  const startEditConversation = (e, conv) => {
    e.preventDefault();
    e.stopPropagation();
    setEditingConversationId(conv.id);
    setEditingConversationTitle(conv.title || 'New Conversation');
  };

  const handleRenameConversation = (e) => {
    e.preventDefault();
    if (editingConversationTitle.trim() && editingConversationId) {
  ...
  const cancelEditConversation = (e) => {
    e.preventDefault();
    e.stopPropagation();
    setEditingConversationId(null);
    setEditingConversationTitle('');
  ...
  const renderConversation = (conv) => (
    ...
    >
      {confirmDelete === conv.id ? (
        ...
        <form onSubmit={handleRenameConversation} className="conversation-rename-form">
          <input
            type="text"
            ...
            onKeyDown={(e) => {
              if (e.key === 'Escape') {
                cancelEditConversation(e);
              }
            }}
          />
          <button type="submit" className="rename-btn save" title="Save">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <polyline points="20 6 9 17 4 12"/>
            </svg>
          </button>
          <button type="button" className="rename-btn cancel" onClick={cancelEditConversation} title="Cancel">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          <div className="conversation-actions">
            ...
            {!showArchived && (
              <button
                ...
                onClick={(e) => startEditConversation(e, conv)}
